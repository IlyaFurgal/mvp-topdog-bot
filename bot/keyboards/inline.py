...
_TZ_LABELS = {"UTC+3": "UTC+3 🇷🇺"}


def kb_timezone() -> InlineKeyboardMarkup:
    rows = []
    for i in range(0, len(_TIMEZONE_KEYS), 3):
        row = [
            InlineKeyboardButton(
                text=_TZ_LABELS.get(k, k),
                callback_data=f"reg_tz_{k}",
            )
            for k in _TIMEZONE_KEYS[i:i + 3]
        ]
        rows.append(row)
    return InlineKeyboardMarkup(inline_keyboard=rows)
